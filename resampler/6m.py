...
for ticker in tickers:
    ...
    for file_name in dir_list:
        ...
        df = pd.DataFrame(df)
        data = df.resample('6M', on='Date').agg({'Open': 'first',
                                                 'High': 'max',
                                                 'Low': 'min',
                                                 'Close': 'last',
                                                 'Adjusted_close': 'last',
        ...
        if not os.path.exists('../data/6m/' + ticker):
            # if the demo_folder directory is not present 
            # then create it.
            os.makedirs('../data/6m/' + ticker)

        # Write data in csv file inside data directory
        data.to_csv('../data/6m/' + ticker + "/" + file_name, mode='w', header=True)
# ********* Reads 1 month CSV Resample data to 3 months and store it in data directory for frontend chart **************
